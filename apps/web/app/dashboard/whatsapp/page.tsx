        ...
        <div className="lg:col-span-5 space-y-6">
          <Card variant="glow" className="border-emerald-500/30">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="h-80 overflow-y-auto p-4 rounded-2xl bg-[#091b15] border border-emerald-500/20 text-xs space-y-3 shadow-inner">
                {chatMessages.map((msg, i) => (
                  ...
                  >
                    <div
                      className={`max-w-[85%] rounded-2xl p-3 shadow-md whitespace-pre-line text-xs ${
                        msg.sender === "user"
                          ? "bg-emerald-600 text-white rounded-br-none"
                          : "bg-[#142c23] text-slate-100 rounded-bl-none border border-emerald-500/20"
                      }`}
                    >
                      {msg.sender === "bot" && (
              ...
              <div className="space-y-2">
                <span className="text-[11px] text-slate-400 font-semibold block">Simulate Guest Inbound Reply:</span>
                <div className="flex flex-wrap gap-1.5">
                  <button
                    onClick={() => handleSimulateReply("5")}
                    className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-[11px] text-amber-300 hover:border-amber-400 transition-colors cursor-pointer"
                  >
                    Rate &quot;5 ⭐&quot;
                  </button>
                  <button
                    onClick={() => handleSimulateReply("STOP")}
                    className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-[11px] text-rose-300 hover:border-rose-400 transition-colors cursor-pointer"
                  >
                    Opt-out &quot;STOP&quot;
                  </button>
                  <button
                    onClick={() => handleSimulateReply("Can we get extra ice please?")}
                    className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-[11px] text-slate-300 hover:border-slate-600 transition-colors cursor-pointer"
                  >
                    &quot;Extra ice please&quot;
                  </button>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4 space-y-4">
              <div className="space-y-2.5">
                {[
                  ...
                  return (
                    <div key={item.intent} className="flex items-start gap-3 p-3 rounded-xl bg-slate-900/50 border border-slate-800 text-xs">
                      <Icon className={`h-4 w-4 ${item.color} shrink-0 mt-0.5`} />
                      <div>
                        <p className="font-bold text-white">{item.intent}</p>
              ...
              <div className="p-3 rounded-xl bg-violet-500/5 border border-violet-500/20 text-xs">
                <p className="text-violet-300 font-semibold mb-1">Powered by Gemini AI</p>
                <p className="text-slate-400">Contextual replies based on order history, menu data, and kitchen status — not just keyword matching.</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-8">
              <div className="bg-[#0a0d14] border border-slate-800 rounded-2xl overflow-hidden">
                {/* Chat header */}
                <div className="flex items-center gap-3 p-3 border-b border-slate-800 bg-slate-900/60">
                  <div className="h-8 w-8 rounded-full bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center">
                    <Bot className="h-4 w-4 text-white" />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-white">DineBot — The Grand Bistro</p>
                  ...
                  {chatbotMessages.map((msg, idx) => (
                    <div key={idx} className={`flex ${msg.role === "bot" ? "justify-start" : "justify-end"}`}>
                      <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-xs leading-relaxed whitespace-pre-wrap ${
                        ...
                        {msg.text}
                        <div className={`text-[10px] mt-1 ${ msg.role === "bot" ? "text-slate-500" : "text-violet-200/60" }`}>
                          {msg.time}
                        </div>
                      </div>
                    </div>
                ...
                <div className="px-4 pb-2 flex flex-wrap gap-1.5">
                  {["Reorder my last meal", "Where's my order?", "Show today's specials", "STOP"].map((reply) => (
                    <button
                      ...
                      className="text-[11px] px-3 py-1 rounded-full border border-violet-500/30 text-violet-300 hover:bg-violet-500/10 transition-colors font-medium"
                    >
                      {reply}
                    </button>
                  ))}
                ...
                >
                  <input
                    ...
                    className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white placeholder:text-slate-500 outline-none focus:border-violet-500/50"
                  />
                  <button type="submit" className="p-2 rounded-xl bg-violet-600 hover:bg-violet-500 transition-colors">
