...
import { UtensilsCrossed, Sparkles, ChefHat, Bell, Hotel } from "lucide-react";
...
export interface HospitalityLoaderProps {
  ...
  subtitle?: string;
  /** Variant of the loader: 'cloche' (gourmet platter) | 'bell' (concierge service bell) | 'minimal' */
  variant?: "cloche" | "bell" | "minimal";
    ...
    <div className="flex flex-col items-center justify-center text-center max-w-sm mx-auto px-6 select-none">
      ...
      <div className="relative w-36 h-36 flex items-center justify-center mb-6">
        ...
        <motion.div
          ...
          className="absolute inset-0 rounded-full bg-gradient-to-tr from-emerald-500/20 via-teal-400/20 to-amber-500/10 blur-xl pointer-events-none"
        />
          ...
          <div className="min-h-[2rem] flex items-center justify-center px-2">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800/70 shadow-xs"
              >
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block flex-shrink-0" />
                <span className="text-xs sm:text-sm font-semibold text-emerald-800 dark:text-emerald-300 tracking-tight">
                  {activeMessage}
                </span>
              </motion.div>
            </AnimatePresence>
      ...
      <div className="w-48 h-1.5 rounded-full bg-slate-200 dark:bg-slate-800/80 overflow-hidden relative mt-2">
        {progress !== undefined ? (
          ...
          <motion.div
            ...
            className="w-1/2 h-full bg-gradient-to-r from-emerald-500 via-teal-400 to-emerald-500 rounded-full"
          />
        )}
      </div>

      {/* DineFlow Hospitality Brand Tag */}
